...
size = 0.5
box = ode.GeomBox(space, lengths=(size,size,size))
box.setPosition((5,dist+size/2,0))
box.shape = "box"
...
ball, ballgeom = create_sphere(world, space, 2500, 1)
...
while loopFlag:
    ...
    ball.addTorque((0,0,-10000))
    #ball.addForce((2000,0,0))
    # print(ball.getAngularVel())
    q = ball.getQuaternion()
    qw,qx,qy,qz = q


    roty = math.asin(2*(qw*qy - qz*qx))
    rotz = math.atan2(2*(qw*qz + qx*qy), 1 - 2*(qy**2 + qz**2))
