...
class WheelPrivate {
 ...
 private:
  ...
  rclcpp::Subscription<vehicle_interface::msg::WheelCommand>::SharedPtr
      command_sub_p;
  rclcpp::Publisher<vehicle_interface::msg::WheelState>::SharedPtr
      wheel_state_pub_p;
  ...
  void OnUpdate(const gazebo::common::UpdateInfo& info) {
    constexpr unsigned int z_axis_index = 0;
    ...
    wheel_state_msg.steering_angle_rate =
        steer_joint_p->GetVelocity(z_axis_index);
...
Wheel::Wheel() : impl_p(std::make_unique<WheelPrivate>()) {}
...
void Wheel::Reset() { impl_p->Reset(); }

void WheelPrivate::Load(gazebo::physics::ModelPtr model_p,
  ...
  RCLCPP_INFO(ros_node_p->get_logger(), "Loading Wheel plugin for " + name);

  const std::string steer_joint_name =
      sdf_p->GetElement("steer_joint")->GetAttribute("name")->GetAsString();
  const std::string drive_joint_name =
      sdf_p->GetElement("drive_joint")->GetAttribute("name")->GetAsString();
  ...
  rcpputils::assert_true(steer_joint_p != nullptr,
                         "failed to load steer joint");
  rcpputils::assert_true(drive_joint_p != nullptr,
                         "failed to load drive joint");

  // ros_node_p->create_subscriber()
  const auto command_topic = name + "/command";
  command_sub_p =
      ros_node_p->create_subscription<vehicle_interface::msg::WheelCommand>(
          command_topic,
          qos.get_subscription_qos(command_topic, rclcpp::QoS(1)),
          std::bind(&WheelPrivate::OnCommand, this, std::placeholders::_1));

  const auto state_topic = name + "/state";
  wheel_state_pub_p =
      ros_node_p->create_publisher<vehicle_interface::msg::WheelState>(
