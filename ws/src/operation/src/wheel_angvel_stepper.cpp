...
class WheelAngvelStepper : public rclcpp::Node {
 public:
  explicit WheelAngvelStepper(const rclcpp::NodeOptions& options)
    ...
    timer_p =
        this->create_wall_timer(std::chrono::duration<double>(0.1),
                                std::bind(&WheelAngvelStepper::update, this));
 ...
 private:
  ...
  void update() {
    ...
    if ((time - starttime).seconds() > steptime) {
      wheelstate.angular_velocity = stepvalue;
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(WheelAngvelStepper)
